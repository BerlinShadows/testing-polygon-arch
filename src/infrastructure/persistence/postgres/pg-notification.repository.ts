...
export class PgNotificationRepository implements NotificationRepositoryPort {
    ...
    async findById(id: string): Promise<Notification | null> {
        const result = await this.pg.getPool().query('SELECT * FROM notifications WHERE id = $1', [id]);
        return result.rows.length ? this.mapRowToNotification(result.rows[0]) : null;
    }
