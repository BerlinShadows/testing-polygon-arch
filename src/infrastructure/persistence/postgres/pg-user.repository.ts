...
export class PgUserRepository implements UserRepositoryPort {
    ...
    async findById(id: string): Promise<User | null> {
        const result = await this.pg.getPool().query('SELECT * FROM users WHERE id = $1', [id]);
        return result.rows.length ? this.mapRowToUser(result.rows[0]) : null;
    }

    async findByEmail(email: string): Promise<User | null> {
        const result = await this.pg.getPool().query('SELECT * FROM users WHERE email = $1', [email]);
        return result.rows.length ? this.mapRowToUser(result.rows[0]) : null;
    }
    ...
    async update(user: User): Promise<User> {
      ...
      RETURNING *;
    `;
        const values = [user.email, user.name, user.roles, user.isActive, user.updatedAt, user.id];
        const result = await this.pg.getPool().query(query, values);
        return this.mapRowToUser(result.rows[0]);
    }
    ...
    private mapRowToUser(row: any): User {
        return new User(
            ...
            row.email,
            row.name,
            row.roles, // JSONB → массив автоматически
            row.is_active,
